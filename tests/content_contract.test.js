...
assert.match(stylesSource, /\.daily-guide-groups\s*\{/);
...
const windowsGlassMirrors = routineById("windows-glass-mirrors");
assert.equal(windowsGlassMirrors.frequencyBucket, "fortnightly");
assert.equal(windowsGlassMirrors.frequencyText.en, "Fortnightly");
assert.equal(windowsGlassMirrors.trackingCadence, "fortnightly");
assert.equal(windowsGlassMirrors.trackingAnchor, "2026-07-06");
assert.deepEqual(Array.from(windowsGlassMirrors.legacyTrackingCadences), ["weekly"]);
assert.match(windowsGlassMirrors.summary.en, /interior side/);
assert.match(windowsGlassMirrors.photos[0].caption.en, /Every two weeks/);
const sofaCoversPillows = routineById("sofa-covers-pillows");
assert.equal(sofaCoversPillows.frequencyBucket, "fortnightly");
assert.equal(sofaCoversPillows.frequencyText.en, "Fortnightly");
assert.equal(sofaCoversPillows.trackingCadence, "fortnightly");
assert.equal(sofaCoversPillows.trackingAnchor, "2026-07-06");
assert.deepEqual(Array.from(sofaCoversPillows.legacyTrackingCadences), ["weekly"]);
assert.match(englishText(sofaCoversPillows.mustRemember), /every two weeks/);
const floorMats = routineById("floor-mats");
assert.equal(floorMats.frequencyBucket, "fortnightly");
assert.equal(floorMats.frequencyText.en, "Fortnightly");
assert.equal(floorMats.trackingMode, "checkbox");
assert.equal(floorMats.trackingCadence, "fortnightly");
assert.equal(floorMats.trackingAnchor, "2026-07-06");
assert.deepEqual(Array.from(floorMats.legacyTrackingCadences), ["weekly"]);
assert.match(englishText(floorMats.mustRemember), /downstairs neighbour's clothes/);
...
assert.deepEqual(Array.from(physicalMailbox.photos, (item) => item.src), [
  "assets/routines/physical-mailbox-location.jpg",
  "assets/routines/physical-mailbox-10-133.jpg"
]);
assert.match(physicalMailbox.photos[1].caption.en, /mailbox 10-133/);
...
assert.equal(routineById("cleaning-tools").trackingMode, "none");
assert.equal(routineById("cleaning-tools").trackingCadence, null);
assert.match(routineById("cleaning-tools").trackingExclusionReason, /excluded from Routine Check-in/);
assert.deepEqual(Array.from(routineById("cleaning-tools").legacyTrackingCadences), ["weekly"]);
const curtainSteaming = routineById("curtain-steaming");
assert.equal(curtainSteaming.frequencyBucket, "monthly");
assert.equal(curtainSteaming.frequencyText.en, "Monthly");
assert.equal(curtainSteaming.trackingMode, "checkbox");
assert.equal(curtainSteaming.trackingCadence, "monthly");
assert.equal(curtainSteaming.trackingAnchor, null);
assert.deepEqual(Array.from(curtainSteaming.legacyTrackingCadences), ["fortnightly"]);
assert.match(curtainSteaming.photos[0].caption.en, /Monthly reference/);
